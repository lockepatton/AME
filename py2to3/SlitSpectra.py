...
class SlitSpectra(object):
    ...
    def findcenterwidthBackground(self, verbose=True):
        ...
        def width_func(top, bot): return round(top - bot, 3)
        # find the center of aperture extraction using the input top and bottom of extraction

        def center_func(top, bot): return round((top + bot) * .5, 3)
        ...
        self.extract_centers = list(
            map(center_func, highr_apertures, lower_apertures))
        self.extract_widths = list(
            map(width_func, highr_apertures, lower_apertures))
        ...
        while i < self.n_apertures - 1:
            if (self.extract_centers[i - 1] < self.extract_centers[i]) & (verbose == True):
...
#         wavelengths = wavelengths[wavelengths_within_range_index]
        ...
        def pixel(wave, w1, dw): return round(old_div((wave - w1), dw) + 1, 3)
        ...
        self.extract_centers = list(
            map(pixel, wavelengths, wavelength1, delta_wavelength))
        self.extract_widths = pixel_widths
# extract_backgrounds defined

        def background_func(b1234_, center, trace): return np.sort([round(b1234_[0] + center - trace, 2), round(b1234_[1] + center - trace, 2),
        ...
        def norm_region(trim_region):
            new_region = list(map(g, trim_region, [
                              trim_region[0], trim_region[0], trim_region[2], trim_region[2]]))
            return new_region
...
# extract_names defined
        ...
        self.extract_lowers = list(map(lambda cen, wid: cen - old_div(
            wid, 2) - self.trace_center, self.extract_centers, self.extract_widths))
        self.extract_uppers = list(map(lambda cen, wid: cen + old_div(
            wid, 2) - self.trace_center, self.extract_centers, self.extract_widths))

        self.extract_names = [findname(self.image, self.direction, wave_, c_, self.trace_center, w_)
                              for wave_, c_, w_ in zip(self.wavelength_names, self.extract_centers, self.extract_widths)]
        ...
        def printlen(string, length):
            print(('{i:<%s}' % str(length)).format(i=string), end=' ')
        ...
        printApall('image', 'trace', 'center', 'width',
                   'lower', 'upper', 'file name', 'background')
...
# HERE
    ...
    def allcalc(self, verbose=True):
        ...
        full_center = old_div((self.full_lowest + self.full_highest), 2)
...
# extract_names defined
        ...
        self.extract_lowers = [(x - old_div(y, 2)) - self.trace_center for x,
                               y in zip(self.extract_centers, self.extract_widths)]
        self.extract_uppers = [(x + old_div(y, 2)) - self.trace_center for x,
                               y in zip(self.extract_centers, self.extract_widths)]

        if self.fileBackground == True:
            def background_func(b1, b2, b3, b4, center): return np.sort([round(b1 - center, 2), round(b2 - center, 2),
        ...
        self.extract_names = [findname(self.image, self.direction, c_, self.trace_center, w_)
                              for c_, w_ in zip(self.extract_centers, self.extract_widths)]
...
# SN_name defined
        ...
        self.SN_lower = self.SN_center - \
            old_div(self.SN_width, 2) - self.trace_center
        self.SN_upper = self.SN_center + \
            old_div(self.SN_width, 2) - self.trace_center
        self.SN_background = [round(self.SN_lower, 2) - 8, round(
            self.SN_lower, 2) - 3, round(self.SN_upper, 2) + 3, round(self.SN_upper, 2) + 8]
        self.SN_name = findname(
            self.image, self.direction, self.SN_center, self.trace_center, self.SN_width)
    ...
    def buildbasedatabase(self, database_source=None, verbose=True):
        ...
        with open(self.path + 'database/ap' + self.image, 'w') as file:
            data = data_basefile
            data[1] = "begin	aperture " + self.image + \
                " 1 1225. " + str(self.trace_center) + '\n'
...
#                 data[7]="	high	681. "+str(self.upper[0])+'\n'
        ...
        with open(self.path + 'database/ap' + self.image + '_base', 'w') as file:  # write apIMAGE base file
    ...
    def builddatabase(self, verbose=True):
        ...
        if not self.builtdatabase:
            ...
            # read in current apIMAGE file
            with open(self.path + 'database/ap' + self.image + '_base', 'r') as file:
                image_basefile = file.readlines()
            ...
            with open(self.path + 'database/ap' + self.image, 'w') as file:
                while aperture_id < self.n_apertures + 1:
                    data = image_basefile
                    data[1] = "begin	aperture " + self.image + " " + \
                        str(aperture_id) + " 1225. " + \
                        str(self.center[aperture_id - 1]) + '\n'
                    data[2] = "	image	" + self.image + '\n'
                    ...
                    if self.direction == 'y':
                        data[5] = "	center	" + \
                            str(self.trace_center) + ' 360.' + '\n'
                        data[6] = "	low	" + \
                            str(self.lower[aperture_id - 1]) + ' -359.' + '\n'
                        data[7] = "	high	" + \
                            str(self.upper[aperture_id - 1]) + ' 361.' + '\n'
                    elif self.direction == 'x':
                        data[5] = "	center	" + \
                            str(self.trace_center) + ' 360.' + '\n'
                        data[6] = "	low	" + \
                    ...
                    data[13] = "		sample " + str(b[0]) + ":" + str(
                        b[1]) + "," + str(b[2]) + ":" + str(b[3]) + '\n'
                    file.writelines(data)
                    aperture_id += 1

            if verbose:
                print('Built database with ' +
                      str(aperture_id - 1) + ' apetures.')
                print('')
    ...
    def buildothercoordinatefile(self, transfer_image_to_write, base_channel_points, transfer_channel_points,
                                 ...
                                 makecoordfile=True, verbose=False):
        """
        ...
        values_in_base = np.sort(
            [self.full_lowest] + base_channel_points + [self.full_highest])
        values_in_transfer = np.sort(
            [transfer_full_lowest] + transfer_channel_points + [transfer_full_highest])

        self.full_base_vs_transfer_fit = np.polyfit(
            values_in_base, values_in_transfer, 1)

        if verbose:
            print('>' * 3, self.channel, self.image,
                  transfer_image_to_write, '<' * 3)
        ...
        transfer_values_full = np.polyval(
            self.full_base_vs_transfer_fit, base_values)
        transfer_values_trim = np.polyval(
            self.trim_base_vs_transfer_fit, base_values)

        self.fit_transfer_values_full = transfer_values_full
        self.fit_transfer_values_trim = transfer_values_trim
        ...
        if makecoordfile:
            ...
            transferYvals = np.polyval(
                self.trim_base_vs_transfer_fit, baseYdata)
        ...
        if verbose:
            ...
            fig, ax = plt.subplots(1, 1)
            ax.set_title(self.image + ' ' + transfer_image_to_write +
                         ' - Channel Matching Functions')
            ax.plot(values_in_base, values_in_transfer, 'o', label='to fit')
            ax.plot(base_values, transfer_values_trim,
                    label='trimmed region fit function')
            ax.plot(base_values, transfer_values_full,
                    label='full image fit function')
            if makecoordfile:
                ax.plot(basedata['col2'], transferdata['col2'],
                        'o', label='values for image and transfer image')
    ...
    def reimagedatabase(self, image_to_write, verbose=True):
        ...
        createfile(self.path + 'database/ap' + self.image,
                   self.path + 'database/ap' + image_to_write, verbose)
        ...
        with open(self.path + 'database/ap' + image_to_write, 'w') as file:
            ...
            for aperture_id, start in enumerate(starts):
                ...
                data[start + 1] = data[start +
                                       1].replace(self.image, image_to_write)
                data[start + 2] = data[start +
                                       2].replace(self.image, image_to_write)
            file.writelines(data)
    ...
    def plotImage(self, save_fig=False, verbose=True, x=None, y=None, vmin=0, vmax=250, blackout=False):
        ...
        for i in range(self.n_apertures):
            cen = self.center[i]
            wid = self.width[i]
            image_data[int(cen - old_div(wid, 2)):int(cen + old_div(wid, 2)),
                       0:center_end + 15 * (self.n_apertures - i)] *= .7
            ...
            if blackout:
                image_data[int(cen - old_div(wid, 2)):int(cen + old_div(wid, 2)),
                           0:center_end + 15 * (self.n_apertures - i)] = 50
    ...
    def wave(self, x_pix):
        """
        Inputs x_pixel values, outputs their equivalent in wavelength space based upon object's dispersion values.
        Parameters
        ----------
        x_pix : boolian
            x pixel values array
        """
...
#             return pixel
        ...
        return wavelength(x_pix, self.w1, self.dw)

    def plotSpectra(self, opening_path=None, save_fig=True, verbose=True, x=None, y=None):
        """
...
# defined self.spectra
    ...
    def buildSpectra(self, verbose=False):
        """
        Builds self.spectra for specific wavelength. (usually y direction specific)
    ...
    def plotWavelengthSpectra(self, wavename1, wavename2=None, x=None, y=None, baseline=None, minSubtract=False,
        ...
        if m == None:
            ...
            sum_over_3 = old_div(
                (spectra_n_min + spectra_n_mode + spectra_n_mean), 3)
            ...
            axall.plot(
                x_pix, [spectra_n_min for x_pix_int in x_pix], 'b', alpha=.2, label='min')
            axall.plot(
                x_pix, [spectra_n_mode for x_pix_int in x_pix], 'g', alpha=.2, label='mode')
            axall.plot(
                x_pix, [spectra_n_mean for x_pix_int in x_pix], 'm', alpha=.2, label='mean')
            axall.plot(x_pix, [sum_over_3 for x_pix_int in x_pix],
                       'k', alpha=.2, label='min+mode+mean/3')
...
# GO HERE
            ...
            to_return = [x_pix, self.spectra[n]]

            filenametosave = directory + self.image + \
                '_linespectra_' + str(wavename1) + '.jpg'
        ...
        if m != None:
            ...
            if minSubtract:
                spectra_ratio = old_div(
                    (self.spectra[n] - baseline[0]), (self.spectra[m] - baseline[1]))
            else:
                spectra_ratio = old_div((self.spectra[n]), (self.spectra[m]))

            # plotting scatter colorplot based upon 1st
            colors = cm.Blues(old_div(
                (self.spectra[n] - min(self.spectra[n])), max((self.spectra[n] - min(self.spectra[n])))))
            axall.scatter(x_pix, spectra_ratio, c=colors)

            # plotting spectra_ratio
            axall.plot(x_pix, spectra_ratio, c='b',
                       label='Ratio', linewidth=.5, alpha=.2)
            ...
            true_height = old_div((height_n + height_m), 2)
            y_n = old_div(
                (self.spectra[n] - self.spectra[n].min()), true_height)
            # TO DO test true_height
            y_m = old_div(
                (self.spectra[m] - self.spectra[m].min()), true_height)

            axall.plot(x_pix, y_n, linewidth=.5, c='r',
                       label=n_wavelengthname, alpha=.2)
            axall.plot(x_pix, y_m, linewidth=.5, c='g',
                       label=m_wavelengthname, alpha=.2)

            if x == None:
                # TO DO could be done much better; also fix i vs. i-1
                x = [x_pix.min(), x_pix.max()]

            axall.set_xlim(x[0], x[1])
            axall.set_title(self.image)
            axall.set_ylabel(
                self.wavelength_names[n - 1] + ' / ' + self.wavelength_names[m - 1])
            axall.set_xlabel('Pixel')

            axall.legend(loc=0, bbox_to_anchor=(1, 1))

            filenametosave = directory + self.image + '_linespectra_' + \
                str(wavename1) + '_per_' + str(wavename2) + '.jpg'

            # values to return to user
#            to_return = [x_pix, [y_n, n_wavelengthname], [y_m, m_wavelengthname]]
            to_return = [x_pix, [y_n, self.spectra[n], n_wavelengthname],
                         [y_m, self.spectra[m], m_wavelengthname]]
    ...
    def returnAllSpectra(self, verbose=False):
        ...
        # iterating through all extracted spectral wavelengths
        # adding all to AllSpectra Dictionary
        for wavename in self.wavelength_names:
            n = waveInt(wavename)
            spec = self.spectra[n]
            AllSpectra[wavename] = spec

        try:
            return [x_pix, AllSpectra]
        except:
    ...
    def finalizeFiles(self, t=None, verbose=True):
        ...
        for i, name in enumerate(self.name):
            ap_file_in = self.path + self.image + \
                '.{:04}'.format(i + 1) + '.fits'
            ap_file_out = backupdir + name + \
                '_aper.{:04}'.format(i + 1) + '_' + t
...
def findRADEC(RAh, RAm, RAs, DECd, DECm, DECs, equinox='J2000'):
    RADEC_Array = []
    for RAh_, RAm_, RAs_, DECd_, DECm_, DECs_ in zip(RAh, RAm, RAs, DECd, DECm, DECs):
        if RAs_ < 0:
            RAm_ = RAm_ - 1
            RAs_ = RAs_ + 60
        if DECs_ < 0:
        ...
        if DECm_ >= 60:
            DECd_ = DECd_ + 15
            DECm_ = DECm_ - 60
        RADEC_Array.append(str(RAh_) + 'h' + str(int(RAm_)) + 'm' + str(RAs_) +
                           's +' + str(DECd_) + 'd' + str(int(DECm_)) + 'm' + str(DECs_) + 's')

    c = SkyCoord(RADEC_Array, unit=(u.hourangle, u.deg), equinox=equinox)
    ra = c.ra.deg
    dec = c.dec.deg
    return c, ra, dec
...
def findangdistance2(RA, DEC, RA_array, DEC_array):
    # Assumes Ra, Dec are both in degrees
    # returns distance squared array in degrees between RA DEC and ra/dec arrays and min value within said aray
    dist2_array = []
    for ra, dec in zip(RA_array, DEC_array):
        angdist = ((RA - ra) * np.cos(dec * 2 * np.pi / 360.)
                   )**2 + (DEC - dec)**2
        dist2_array.append(angdist[0])
    return dist2_array, np.argmin(dist2_array)


def localdistance(distanceaway, angle):
    angle = angle * u.deg
    ang_dist = distanceaway * np.sin(angle)  # sin(theta) = answer / D
    return ang_dist


def noNanArray(array):
    return [y for y in array if math.isnan(y) == False]


def noNanArrays(array_to_reassign, array_to_remove_nans):
    indicies_to_keep = [a for a, y in enumerate(
        array_to_remove_nans) if math.isnan(y) == False]

    reassigned_array = np.array(array_to_reassign)[indicies_to_keep]
    removed_nans_array = np.array(array_to_remove_nans)[indicies_to_keep]

    return reassigned_array, removed_nans_array
