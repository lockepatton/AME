...
def findRADEC(RAh,RAm,RAs,DECd,DECm,DECs, equinox='J2000'):
    ...
    for RAh_,RAm_,RAs_,DECd_,DECm_,DECs_ in zip(RAh,RAm,RAs,DECd,DECm,DECs):
        ...
        if DECm_ >= 60:
            DECd_ = DECd_ + 15
            DECm_ = DECm_ - 60
        RADEC_Array.append(str(RAh_)+'h'+str(int(RAm_))+'m'+str(RAs_)+'s +'+str(DECd_)+'d'+str(int(DECm_))+'m'+str(DECs_)+'s')
...
def findangdistance2(RA, DEC, RA_array, DEC_array):
    ...
    for ra, dec in zip(RA_array,DEC_array):
        angdist = ((RA-ra)*np.cos(dec*2*np.pi/360.))**2 + (DEC-dec)**2
...
def noNanArrays(array_to_reassign,array_to_remove_nans):
    indicies_to_keep = [a for a,y in enumerate(array_to_remove_nans) if math.isnan(y) == False]
