...
#GO HERE
        ...
        if m != None:
            ...
            axall.plot(x_pix, spectra_ratio, linewidth=.5)
            
            print np.mean(self.spectra[n])/np.mean(self.spectra[m])
            #plotting min-subtracted base spectra normalized by height (min 0 to max 1)
            height_n = (self.spectra[n]-self.spectra[n].min()).max()
            height_m = (self.spectra[m]-self.spectra[m].min()).max()
            y_n = (self.spectra[n]-self.spectra[n].min())/height_n
            y_m = (self.spectra[m]-self.spectra[m].min())/height_m
            axall.plot(x_pix, y_n, linewidth=.5,c='r',alpha=.2)
            axall.plot(x_pix, y_m, linewidth=.5,c='r',alpha=.2)
